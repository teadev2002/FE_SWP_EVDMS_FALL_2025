    ...
    useEffect(() => {
        const fetchVehicleData = async () => {
            ...
            try {
                ...
                const brand = brandsData.find(b => b.brandId === vehicle.brandId) || {};
                // Sum only allocated quantities for this vehicle (exclude HQ null)
                const quantity = getTotalQuantityAcrossStores(parseInt(id), storagesData);
                let stockType = 'out-of-stock';
                let stockText = quantity > 0 ? `${quantity} Available across all locations` : 'Not Available';
                if (quantity > 1) {
                    stockType = 'available';
                } else if (quantity === 1) {
                    stockType = 'limited';
