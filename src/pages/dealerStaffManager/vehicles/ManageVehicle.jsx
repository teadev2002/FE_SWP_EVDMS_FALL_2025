...
//       return null;
//     } catch (error) {
//       console.error('Error parsing dealerInfo from localStorage:', error);
//       return null;
//     }
//   };

//   // Fetch vehicles by storeId
//   useEffect(() => {
//     const fetchData = async () => {
//       setLoading(true);
...
//       try {
//         const [vehicleData, brands, storages] = await Promise.all([
//           ManageVehicleService.getAllVehicleByStoreId(storeId), // API mới
//           ManageVehicleService.getAllBrands(),
//           ManageVehicleService.getAllStorages(),
//         ]);
...
//   // Lọc dữ liệu chỉ theo search text
//   const processedData = useMemo(() => {
//     let data = vehicles;
...
//               enterButton
//               allowClear
//               aria-label="Tìm kiếm xe theo model"
//               style={{ marginBottom: 16 }}
//             />
...
//               <Col>
//                 <Text>
//                   Hiển thị {startIndex} đến {endIndex} trong tổng {totalVehicles} xe
//                 </Text>
//               </Col>
//             </Row>
...
//               bordered
//               loading={loading}
//               aria-label="Bảng danh sách xe"
//             />

//             {/* Actions */}
//             <div className="actions" role="toolbar" aria-label="Hành động bảng">
//               <Button icon={<DownloadOutlined />} onClick={exportCSV} aria-label="Xuất danh sách ra CSV">
//                 Export CSV
//               </Button>
//               <Upload {...uploadProps}>
//                 <Button icon={<UploadOutlined />} aria-label="Tải lên dữ liệu xe mới">
//                   Upload New Vehicle
//                 </Button>
//               </Upload>
//               <Button
...
//                 onClick={() => setIsCompareModalVisible(true)}
//                 disabled={selectedRowKeys.length === 0 || selectedRowKeys.length > 3}
//                 aria-label={`So sánh ${selectedRowKeys.length} xe đã chọn`}
//               >
//                 So sánh ({selectedRowKeys.length}/3)
//               </Button>
//             </div>
//           </Card>
//         </Col>
...
const ManageVehicle = () => {
  ...
  const [selectedVehicles, setSelectedVehicles] = useState([]);
  const [isDetailModalVisible, setIsDetailModalVisible] = useState(false);
  const [selectedVehicle, setSelectedVehicle] = useState(null); // ← Dữ liệu đầy đủ từ API
  const [isCompareModalVisible, setIsCompareModalVisible] = useState(false);
  const [sortConfig, setSortConfig] = useState({ field: null, order: null });
  const [searchText, setSearchText] = useState('');
  const debouncedSearchText = useDebounce(searchText, 300);
  ...
  // Lấy quantity theo storeId + vehicleId + brandId
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn('Lỗi lấy danh sách thương hiệu:', error);
        }

        setBrandsData(brands);

        if (vehicleData.length === 0) {
          toast.info('Cửa hàng chưa có xe nào.');
          setVehicles([]);
          setFilteredVehicles([]);
          setLoading(false);
          return;
        ...
        const enrichedVehicles = await Promise.all(
          vehicleData.map(async (item) => {
            ...
            return {
              ...
              onPromotion: false,
              vehicleId: item.vehicleId, // ← Dùng để gọi API chi tiết
            };
          })
        );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          type="link"
          onClick={() => openDetailModal(record.vehicleId)} // ← GỌI API CHI TIẾT
          loading={loading}
        >
          Detail
        </Button>
  ...
  return (
    <div className="ev-dashboard" role="main">
      ...
      <Modal
        ...
        open={isCompareModalVisible}
        onCancel={() => setIsCompareModalVisible(false)}
        footer={[<Button key="close" onClick={() => setIsCompareModalVisible(false)}>Đóng</Button>]}
        width="90%"
      >
        <Row gutter={16}>{compareContent}</Row>
      </Modal>
